...
class PromptConfigQueries:
    """SQL query constants for prompt configuration operations (now merged with system_prompts)."""
    ...
    GET_BY_ID = """
        SELECT 
            ...
            dc.engine_type
        FROM system_prompts sp
        LEFT JOIN db_connections dc ON sp.connection_id = dc.id
        WHERE sp.id = %s
    """
