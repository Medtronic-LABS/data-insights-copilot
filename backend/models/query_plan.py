...
class TimeRange(BaseModel):
    ...
    relative: Optional[str] = Field(None, description="Relative time like 'last_7_days', 'this_month', 'last_quarter'")
...
class QueryPlan(BaseModel):
    ...
    select_columns: List[str] = Field(default_factory=list, description="Columns to SELECT (non-aggregated)")
...
class SchemaLinkResult(BaseModel):
    ...
    columns: List[str] = Field(default_factory=list, description="Matched column names (table.column format)")
    default_filters: List[Filter] = Field(default_factory=list, description="Default filters to apply")
    confidence: float = Field(1.0, description="Linking confidence score")
    
    # Mapping of NL terms to schema elements
    term_mappings: dict = Field(default_factory=dict, description="NL term -> schema element mappings")
