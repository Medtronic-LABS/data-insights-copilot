...
# Critique prompt is loaded dynamically in __init__
...
class SQLCritiqueService:
    def __init__(self, schema_graph=None):
        ...
        import os
        try:
            template_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "agent_spec", "prompt_templates", "reflection_critique.md")
            with open(template_path, "r") as f:
                critique_prompt = f.read()
        except Exception:
            critique_prompt = "You are a Senior SQL Expert and Security Auditor. Validate the SQL query."
