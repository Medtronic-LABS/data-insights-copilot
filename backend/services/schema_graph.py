...
from backend.models.query_plan import (
    ColumnInfo, ForeignKey, TableInfo, JoinStep, JoinPath
)
...
class SchemaGraph:
    ...
    def __init__(self, engine: Engine, schema_name: str = "public", excluded_tables: Optional[List[str]] = None):
        """
        ...
        self._adjacency: Dict[str, Dict[str, List[ForeignKey]]] = defaultdict(lambda: defaultdict(list))
    ...
    def _introspect_foreign_keys(self, conn):
        ...
        try:
            ...
            logger.warning(f"Failed to introspect foreign keys: {e}. Join path computation will be limited.")
    ...
    def to_prompt_format(self, tables: Optional[List[str]] = None, include_sample_values: bool = False) -> str:
        """
        ...
        if len(target_tables) > 1:
            ...
            if join_paths:
                ...
                for jp in join_paths:
                    ...
                    parts.append(f"  {jp.source_table} → {jp.target_table}: " + " → ".join(path_desc))
