...
Replaces the naive keyword-matching in `_get_relevant_schema()` with
structured entity linking that combines:
...
from backend.models.query_plan import (
    SchemaLinkResult, Filter, ComparisonOperator
)
...
class SchemaLinker:
    ...
    def _match_tables_synonym(self, question_lower: str) -> tuple:
    ...
    def _match_columns(self, question_lower: str) -> tuple:
    ...
    def _parse_filter_string(self, filter_str: str, table: str) -> Optional[Filter]:
        ...
        try:
            ...
            match = re.match(r'(\w+)\s*(=|!=|>|>=|<|<=|LIKE|ILIKE|IN|IS NULL|IS NOT NULL)\s*(.*)', 
