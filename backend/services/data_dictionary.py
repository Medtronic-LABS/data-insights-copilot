...
# Default path for the data dictionary config
_DEFAULT_CONFIG_PATH = Path(__file__).parent.parent / "config" / "data_dictionary.yaml"
...
class DataDictionary:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        Args:
            config_path: Path to data_dictionary.yaml. Uses default if not provided.
        """
    ...
    def _load(self):
        ...
        if not self._config_path.exists():
            logger.warning(
                f"Data dictionary config not found at {self._config_path}. "
                "Using empty dictionary. Create the file to enable semantic enrichment."
            )
            return
        ...
        try:
            ...
            total = (
                len(self._business_definitions) + 
                len(self._metric_templates) + 
                len(self._synonyms) +
                sum(len(v) for v in self._default_filters.values())
            )
            logger.info(
