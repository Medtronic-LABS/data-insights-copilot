...
## QA Debug Observability

The Data Insights Copilot includes professional-grade debugging tools directly in the Chat UI and API to ensure every agent response is reproducible and auditable.

### Runtime Meta-Tags (QA Debug)
Every message generated by the AI includes hidden (or togglable) metadata that captures the exact configuration at the moment of execution:

- **Model Identity**: The exact model used (e.g., `gpt-4o-2024-05-13`).
- **Inference Parameters**: Temperature, Top-P, and Max Tokens.
- **Intent Resolution**: The detected user intent (e.g., `SQL_QUERY`, `RAG_SEARCH`, `GREETING`).
- **Context Window**: Total tokens included in the prompt and retrieval window.

### Visibility & Traceability
The UI provides an "Insights" or "Debug" panel for each message:
- **Chain-of-Thought**: View the raw internal reasoning steps taken by the AI.
- **Generated SQL**: Inspect the SQL query BEFORE and AFTER reflection.
- **Retrieval Context**: View the exact document chunks injected into the prompt.
- **Latency Breakdown**: Time spent on intent detection, retrieval, generation, and formatting.

### Reproducibility
By capturing these parameters, engineers can copy the exact configuration to playground environments to reproduce and fix hallucinations or performance bottlenecks.
