-- Migration: Add operational configuration settings for embedding pipeline
-- Created: 2026-03-02
-- 
-- This migration moves operational/business-logic settings from embedding_config.yaml
-- to the database so they can be managed via the frontend UI.
-- 
-- Categories added:
-- - 'data_privacy': PII column exclusions and table exclusions
-- - 'medical_context': Clinical terminology mappings
-- - 'chunking': Detailed chunking strategy settings

-- ============================================================================
-- Data Privacy Settings (PII Stripping Rules)
-- ============================================================================
INSERT OR IGNORE INTO system_settings (category, key, value, value_type, description) VALUES
('data_privacy', 'global_exclude_columns', '["first_name", "last_name", "phone_number", "date_of_birth", "qr_code", "mother_name", "father_name", "mother_occupation", "father_occupation", "mother_educational_qualification", "father_educational_qualification", "password", "national_id"]', 'json', 'Columns to exclude globally from embedding (PII protection)'),

('data_privacy', 'exclude_tables', '["audit", "awsdms_ddl_audit", "deleted_site_program", "duplicate_profile", "email_template", "flyway_schema_history", "form_meta_ui", "offline_log", "outbound_email", "outbound_sms", "shedlock", "site_report_log", "sms_template", "sms_template_values", "updated_duplicate_patient_profile", "user_token"]', 'json', 'Tables to exclude entirely from embedding (system/non-clinical tables)'),
...
-- ============================================================================
-- Medical Context Settings (Clinical Terminology Enrichment)
-- ============================================================================
INSERT OR IGNORE INTO system_settings (category, key, value, value_type, description) VALUES
...
}', 'json', 'Medical terminology mappings for clinical column enrichment'),
...
-- ============================================================================
-- Enhanced Chunking Settings (Parent/Child Strategy)
-- ============================================================================
INSERT OR IGNORE INTO system_settings (category, key, value, value_type, description) VALUES
('chunking', 'parent_chunk_size', '800', 'number', 'Parent chunk size for hierarchical chunking'),
('chunking', 'parent_chunk_overlap', '150', 'number', 'Parent chunk overlap'),
('chunking', 'child_chunk_size', '200', 'number', 'Child chunk size for fine-grained retrieval'),
('chunking', 'child_chunk_overlap', '50', 'number', 'Child chunk overlap'),
...
-- ============================================================================
INSERT OR IGNORE INTO system_settings (category, key, value, value_type, description) VALUES
('vector_store', 'type', '"chroma"', 'string', 'Vector store type: chroma, pinecone, qdrant'),
('vector_store', 'default_collection', '"healthcare_advanced"', 'string', 'Default collection name for embeddings');
